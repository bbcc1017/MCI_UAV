...
class ShinHeuristicRule:
    ...
    def in_flight(self, obs) -> np.ndarray:
        """[고속화 S2-7] 병원별 이송중 대수 — EventManager 증분 카운터가 있으면 재사용.

        `RuleManager.Rule.in_flight` 와 같은 계약이다(값 동일, 없으면 전수 계산 폴백).
        """
    ...
    def init_with_scenario(self, scenario):
        ...
        self._en_manager = scenario["EntityManager"]   # [고속화 S2-7]
        hp = props["hospital"]
    ...
    def _load(self, obs) -> np.ndarray:
        if _cap_gate_is_occ():
            return (
                ...
                + self.in_flight(obs)   # [고속화 S2-7] 증분 카운터 재사용(값 동일)
            )
