...
- OSM via Overpass: `osm_roads2.py` (lanes/oneway/class → detailed roads), `osm_features.py` (signals/crossings/bus stops), `osm_areas.py` (park/green/water polygons), `osm_poi.py` (hospital/school/fire/police/fuel). All rotate 3 Overpass mirrors to dodge 429, attribute features to a district by **center-point-in-polygon**, are resumable (skip existing output, `--force`), and write compact text to `tools/nationwide/<kind>/<name>.txt`. `osm_roads2.py footway` → `nationwide/footways/<name>.txt` (보행로/인도, roads2와 분리 레이어; `ImportFootwaysOne`이 `_Sidewalks` 콘크리트 리본 베이크, 주행망 제외).
