...
입력:
  tools/seoul_pilot/_work/scenario/(<lat>,<lon>)/
    hospital_info.csv, amb_station_info.csv, uav_info.csv, patient_info.csv,
    # (구 hospitals.csv/amb_bases.csv/*_road.csv 폴백)
...
def main() -> int:
    ...
    # Phase 1: 통합/카운트 포맷 우선, 구 이름/구 포맷 폴백
    def _pick(*names):
        for n in names:
            if (SCENARIO_DIR / n).exists():
                return SCENARIO_DIR / n
        return SCENARIO_DIR / names[0]
    hospitals = read_csv(_pick("hospital_info.csv", "hospitals.csv", "hospital_info_road.csv"))
    ambs = read_csv(_pick("amb_station_info.csv", "amb_bases.csv", "amb_info_road.csv"))
    uavs = read_csv(_pick("uav_info.csv", "uav.csv"))
