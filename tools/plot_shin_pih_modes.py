#!/usr/bin/env python3
"""대표점 250개에서 Shin PIH 운용방식별 PDR_woG를 그린다."""
...
OUTPUT_PATH = RESULT_ROOT / "analysis/shin_pih_mode_comparison_seed0_29.png"
...
def calculate_rule_statistics() -> tuple[np.ndarray, np.ndarray]:
    ...
    for path in files:
        with np.load(path, allow_pickle=True) as data:
            values = data["values"]
        if values.shape[0] < 12 or values.shape[1] < N_EPISODES:
...
def plot() -> None:
    ...
    for idx, mean in enumerate(means):
        ...
        short_bar = mean < 0.21
        ax.text(
            mean - (0.002 if short_bar else 0.017),
            idx,
            f"{mean:.4f}\nHEUR 대비 {abs(pct):.2f}% {direction}",
            ha="right",
            va="center",
            color="white",
            fontsize=16 if short_bar else 19,
            linespacing=1.45,
            zorder=5,
        )
