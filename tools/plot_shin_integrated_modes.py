#!/usr/bin/env python3
"""대표점 250개에서 Shin Integrated 운용방식별 PDR_woG를 그린다."""
...
OUTPUT_PATH = RESULT_ROOT / "analysis/shin_integrated_mode_comparison_seed0_29.png"
...
def calculate_rule_statistics() -> tuple[np.ndarray, np.ndarray]:
    ...
    for path in files:
        with np.load(path, allow_pickle=True) as data:
            values = data["values"]
        if values.shape[0] < 16 or values.shape[1] < N_EPISODES:
...
def plot() -> None:
    ...
    fig.suptitle(
        "Shin Integrated 휴리스틱: 이송수단 운용방식별 성능",
        fontsize=30,
        y=0.972,
    )
    fig.text(
