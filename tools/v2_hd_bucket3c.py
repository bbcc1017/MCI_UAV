...
"""v2 HD맵 스테이지3c 버킷터 — 신호등(점)/과속방지턱(폴리곤)/가드레일(선) → 1km 타일별.
 ...
 · 신호등(C1_TRAFFICLIGHT, Point 7,115) → 타일 버킷 + 1.5m 그리드 디듀프 → `hd_signals/tile.txt`(`lat lon z`)
 · 과속방지턱(C4_SPEEDBUMP, MultiPolygon 360) → centroid 버킷 → `hd_speedbumps/tile.txt`(`lat lon z…` 외곽링)
 · 가드레일(C3_VEHICLEPROTECTION, MultiLineString 10,294) → 타일 클리핑 → `hd_guardrails/tile.txt`(`lat lon z…`)
좌표=EPSG:4326 lon lat z(2026-07-11 도로 3D 전환: z 방출 — 고가 위 가드레일 실높이).
실행: UAV env python tools/v2_hd_bucket3c.py --region seoul_gangnamgu
"""
...
def main():
    ...
    for f in load("lt_p_c1trafficlight", reg):
        ...
        if key in seen:
            continue
        seen.add(key)
        sig.setdefault((int(E // 1000), int(N // 1000)), []).append(f"{lat:.7f} {lon:.7f} {z:.2f}")
        ns += 1
