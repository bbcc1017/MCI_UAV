...
- **λ 는 공식이다** — `λ ≈ 19 × (관측 치료시간 ÷ 정식 치료시간)^1.12`. 폐루프 시뮬이 필요 없다.
  (선형 근사 `^1.0` 은 6조건 중 4곳만 시뮬 최적과 같은 팔이고 최대 손해가 판정선의 0.98배다.)
- **공식이 없는 축은 등급 임계 하나**(`yhold ∈ {0,2,4,8}`). 폐루프의 남은 일이 여기다.
- **병목은 계산도 통계도 아니라 입력이다.** 현장 예산은 필요량의 15~31배라 계산·통계 모두 남는다.
  결정하는 것은 "치료 회전율을 얼마나 맞게 넣었는가" 하나다(허용 오차 배율 `e ∈ [0.85, 1.24]`).
- **test750 판정 완료(2026-09-13)** — 채택 카드는 `CARD_V22`(`cardt2:18,3,0,0,0,hingerate,on,on,on`,
  0.137897, 격차 회수 103.3%). 최강 교사와는 **실용적 동률**이며 우열을 주장하지 않는다
  (마진 +0.00094 < 학습 시드 잡음 sd 0.00114).
