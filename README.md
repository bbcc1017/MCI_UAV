...
적용을 계기로 **3-Phase 재설계**를 완료했다.

> **진행 현황 (2026-06-24)**: 재설계 후 **실규모 실험 대부분 완료**. ① RL 36런(전국 occ/psent 5M + 시도17 occ/psent 2M) 학습·검토 완료. ② 고정시나리오 **4부류 = {시군구,시도}×{OSRM,Kakao}** 중 **3부류 휴리스틱(occ+psent) 집계 완료**(시군구 Kakao만 생성 진행중, 키 로테이션). ③ ep_rew_mean 그래프 4개, **VIPER 전국 occ/psent 트리 증류** 완료. **핵심결론: 통신단절(occ↔psent) 비용 ≈ 0** — 병원 용량이 부하의 6~15배라 occ/psent 게이트가 거의 미바인딩. 실험 드라이버 `tools/exp_drivers/`, 종합 리포트 `docs/RL_occ_siteonly_report_2026-06-24.md`.
